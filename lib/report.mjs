...
// Printing a verdict is reporting, and the exit code is part of it.
export function reportFindings(findings, options, baselineSkipped = 0) {
  if (options.json) {
    ...
    if (baselineSkipped > 0) {
      ...
      console.log(`(${baselineSkipped} pre-existing ${plural} accepted by the baseline)`);
    }
  }
  const { errors, warnings } = countBySeverity(findings);
