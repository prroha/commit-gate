...
// Where a whole file's current content lives, which is not the same place for
// every check: a staged check means the index, a range means the revision the
// range ends at, and everything else means the files on disk. Symbols, so a
// branch named "index" can never be mistaken for one of them.
export const INDEX = Symbol("index");
export const WORKING_TREE = Symbol("working tree");

// A file absent from that revision is not an error here: the line count is an
// input to one advisory rule, and a rule with nothing to measure says nothing.
export function fileLineCount(path, root, source = INDEX) {
  try {
    if (source === WORKING_TREE) {
      return toLines(readFileSync(join(root, path), "utf8")).length;
    }
    const revision = source === INDEX ? `:${path}` : `${source}:${path}`;
    return toLines(git(["show", revision], root)).length;
  } catch {
    return null;
  }
}
