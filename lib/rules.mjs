...
import { missingReason, suppressedRule } from "./suppression.mjs";
...
// Every extension whose comment style `commentBody` knows, so a language the
// comment rules can read is a language they are given.
const CODE_EXTENSIONS = /\.(m?[jt]sx?|py|go|rb|java|cs|kt|swift|rs|php|sh|ya?ml|tf|pl)$/;
const TYPESCRIPT = /\.m?tsx?$/;
// `console` is a JavaScript object, so the rule is a JavaScript rule: a `run:`
// step in a workflow that prints with node is a script doing its job.
const CONSOLE_LANGUAGES = /\.m?[jt]sx?$/;
...
export function checkCode(path, lines, config, fileLines) {
  ...
  for (const [index, { line, text }] of lines.entries()) {
    ...
    // A comment-only directive belongs to the comment rules, which see the
    // lines this loop skips; here it is a directive trailing a line of code.
